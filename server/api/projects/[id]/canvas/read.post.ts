...
export default defineEventHandler(async (event) => {
  ...
  return {
    nodes: rows.filter(row => row.assetId !== '__viewport__').map(row => ({ id: row.assetId, hidden: row.hidden, x: row.x, y: row.y, width: row.width, height: row.height, ...(row.layerState ? { layerState: row.layerState } : {}) })),
