...
export async function proxyAgentRequest(event: H3Event) {
  ...
  if (method === 'POST' && path === '/v1/chat' && !publicServiceStatus().connected) {
    throw createError({ statusCode: 503, statusMessage: 'Configure and test a language model (OpenRouter, DeepSeek, MiMo or GLM) and at least one image provider (fal or Ark) before sending a message.', data: { code: 'SERVICES_NOT_CONNECTED' } })
  }
