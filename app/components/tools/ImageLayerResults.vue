...
<template>
  <div v-if="job.layers?.length" class="flex w-full min-w-0 flex-col gap-3">
    <ToolsImageLayerStackPreview :job="job" />
    <div class="flex min-w-0 items-start gap-4 overflow-x-auto pb-2" tabindex="0" role="region" aria-label="Individual image layers">
      <button
        v-for="(url, index) in job.resultUrls"
        :key="`${index}-${url}`"
        type="button"
        class="w-44 shrink-0 overflow-hidden rounded-xl border border-border bg-card text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary"
        :aria-label="`View ${layerName(index)}`"
        @click="open({ url, kind: 'image', alt: layerName(index) })"
      >
        <div class="flex h-44 items-center justify-center bg-muted/30 p-3">
  ...
  <div v-else class="flex w-full min-w-0 items-start gap-4 overflow-x-auto pb-2" tabindex="0" role="region" aria-label="Image layers">
    <button
      v-for="(url, index) in job.resultUrls"
      :key="`${index}-${url}`"
      type="button"
