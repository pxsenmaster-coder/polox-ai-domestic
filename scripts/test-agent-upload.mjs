...
function harness({ storageFailure = false, falKey = 'test-key', falFailure = false } = {}) {
  const writes = []
  const source = readFileSync(new URL('../server/agent/upload.ts', import.meta.url), 'utf8').replace(/^import .*\n/gm, '')
  const context = vm.createContext({ exports: {}, readServiceSettings: () => ({ falKey }), uploadFalFile: async () => {
    if (falFailure)
      throw new Error('fal unavailable')
    return 'https://cdn.fal.media/upload.png'
  }, crypto: { randomUUID: () => 'unique-upload' }, saveMediaFile: async (...args) => {
    writes.push(args)
    if (storageFailure)
...
test('Agent uploads the original bytes to local storage and uses the fal CDN when configured', async () => {
  const h = harness()
...
test('Agent uploads stay usable without a fal key', async () => {
  const h = harness({ falKey: '' })
  const url = await h.upload('session-1', { bytes: new Uint8Array([1, 2, 3]), mime: 'image/png' })
  assert.equal(url, 'https://media.example.com/agent-lab/session-1/unique-upload.png')
})
test('fal upload failures fall back to the saved local URL', async () => {
  const h = harness({ falFailure: true })
  const url = await h.upload('session-1', { bytes: new Uint8Array([1, 2, 3]), mime: 'image/png' })
  assert.equal(url, 'https://media.example.com/agent-lab/session-1/unique-upload.png')
})
...
test('upload handler records the resolved upload URL as the session attachment', async () => {
  ...
  const context = vm.createContext({
    exports: {},
    uploadFalFile: async () => 'https://cdn.fal.media/upload.png',
